...
class Readout(utility_functions):
    def __init__(self, rf, ch, rdout_config, rdout_mem_config, top_config, o1, hw_config):
        ...
        self._nco_freq = rdout_config["LO freq"]
        self._nco_phase = rdout_config["phase"]
        self._nyquist_zone = rdout_config["nyquist_zone"]
        ...
        self._trigger_width = self.ns_to_cycles(top_config.trigger_width)
        self._trigger_delay = self.ns_to_cycles(top_config.trigger_delay)
        ...
        self._dma = Dma(o1, ch, rdout_config, top_config,hw_config)
    ...
    def get_adc_src(self):
        return self.get_bitfield(self._conf_param_base_addr, ADC_SRC_SEL_BIT_OFFSET, 1, self._ch)
    ...
    def set_ila_sel(self, val): #This is for debuggng only and same for all the readout channels:: Should find a better place to call it to avoid redundancy
        self.set_bitfield((self._conf_param_base_addr + ADC_ILA_SEL_OFFSET), 0, 2, 0, val) #channel value is passed as 0, as these are a single param for all channels
    def set_readout_update(self, val):
        self.set_bitfield((self._conf_param_base_addr + ADC_UPDATE_OFFSET), 0, 1, 0, val) #channel value is passed as 0, as these are a single param for all channels
    def get_readout_update(self, val):
        self.get_bitfield((self._conf_param_base_addr + ADC_UPDATE_OFFSET), 0, 1, 0)    #channel value is passed as 0, as these are a single param for all channels
    def dma_streamer_thread(self):
        #self._start_reg.mmio.write(0x0, 1)
        self.set_bitfield(self._start_fifo_reg_addr, 0, 1, self._ch, 1)
        i=0
        self._top_config.dma_trf_done_flag[self._ch] = 0
        self._dma._sync_buffer[0][0] = 0
        while True:
            while True:
                # print("Hello, i")
            ...
            self.set_bitfield(self._start_fifo_reg_addr, 0, 1, self._ch, 0)
#             for i in range(self._dma._num_desc):
                # np.savetxt(csvfile,output_buffer[i], fmt='%08x')
                #self._dma._data_buffer[i][0] = i
            self.jj = (numpy.mean(self._dma._data_buffer, axis = 0)).astype(numpy.int64)
            #self.jj = (numpy.mean(self.jj, axis = 0)).astype(numpy.int64)
                #self._streamer._sock.sendto(self._dma._data_buffer[i], (self._streamer._host, self._streamer._port))
            self._streamer._sock.sendto(self.jj, (self._streamer._host, self._streamer._port))
            #logging.debug('xfr_completed_outer %d for channel %d ', self._dma._xfr_completed_outer, self._ch)

            if(self._dma._xfr_completed_outer == self._readout_xfr_count):
                break
            sleep(.1)
            self.set_bitfield(self._start_fifo_reg_addr, 0, 1, self._ch, 1)
...
class IIRFilter(utility_functions):
    ...
    def set_iir_order(self, order):
        self.set_bitfield(self._conf_param_base_addr, ADC_IIR_ORDER_BIT_OFFSET , 2, self._ch, order)
    def get_iir_order(self):
        return self.get_bitfield(self._conf_param_base_addr, ADC_IIR_ORDER_BIT_OFFSET , 2, self._ch)


class Dma(utility_functions):
    def __init__(self, o1, ch, rdout_config, top_config,hw_config):
        ...
        self._data_len = self.ns_to_cycles(top_config.trigger_width)
        ...
        self._sync_buffer = allocate(shape=(1, 2,), dtype=numpy.uint32, cacheable=False)
